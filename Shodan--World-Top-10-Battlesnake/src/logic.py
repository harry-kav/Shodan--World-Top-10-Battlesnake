...
def flood_recursive(start_pos, obstacles, width, height, depth, hazards):
    ...
    def fill(pos, obstacles, depth, hazards, start_depth):
        ...
        if pos in obstacles:
            ...
            elif pos['x'] >= 3 and pos ['x'] <= 7 and pos['y'] >= 3 and pos['y'] <=7 and depth == start_depth or depth == start_depth - 1:
...
def choose_move(data: dict) -> str:
    ...
    for snake in snakes:
        if snake['head'] != my_head:
            ...
            if len(new_possible_moves) != 0:
                ...
                if len(old_possible_moves)!=0:
                  return random.choice(old_possible_moves)
                return random.choice(original_moves)
            ...
            #remove head moves if smaller than another snake
            if data['you']['length'] < snake['length'] or get_distance(my_head, snake['head']) > 2:
    ...
    food_move = 'none'
    closest_food, food_dist = get_closest_food(my_head, data['board']['food'],
                                        hazards)
    if (data['turn'] <= 30 or data['you']['health'] < 60 or data['you'][
            'length'] < 15 or food_dist < 4) and data['board']['food'] != []:
    ...
    if food_move != 'none' and data['you']['health'] < 20 and data['board']['food'] != []:
        move = food_move
    else:
        ...
        if data['turn'] < 20 or data['you']['length'] < 15:
            depth = 9
            food_space_val = data['you']['length'] + 3
            attack_space_val = 8
        else:
            depth = 13
            if data['you']['length'] < 15:
                ...
                food_space_val = data['you']['length']
        ...
        #if an enemy can close a gap take off spaces
        for snake in snakes:
          pos = snake['head']
          if my_head != pos and get_distance(my_head,pos)<11:
            print('checking edgepen')
            print(pos)
            print(my_head)
            #don't get trapped on the left or right
            if (pos['x'] == 1 and my_head['x']== 0):
              print('edge penalty updown')
              if pos['y'] > my_head['y']:
                up_spaces -=8
              else:
                down_spaces -=8
            elif (pos['x'] == board_width -1 and my_head['x'] == board_width):
              print('edge penalty updown')
              if pos['y'] > my_head['y']:
                up_spaces -=8
              else:
                down_spaces -=8
            #don't get trapped top or bottom
            elif (pos['y'] == 1 and my_head['y']== 0):
              print('edge penalty leftright')
              if pos['x'] > my_head['x']:
                right_spaces -=8
              else:
                left_spaces -=8
            elif (pos['y'] == board_height - 1 and my_head['y'] == board_height):
              print('edge penalty leftright')
              if pos['x'] > my_head['x']:
                right_spaces -=8
              else:
        ...
        #try and attack if health is greater than 40 and you are the biggest snake- or if there are only 2 snakes left(less brave in this case)
        bravery = 0
        if is_biggest == True and data['you']['health'] > 40 and data['you']['length']>=10 and len(snakes)>1:
          bravery = 6
          dist_for_interest = 20
        elif len(snakes) <=2 and data['you']['health'] > 60 and data['you']['length']>=10 and len(snakes)>1:
          bravery = 2
          dist_for_interest = 2
        #attack mode
        if bravery >0:
          #bravery = 5
          #dist_for_interest = 10
          snake_heads = []
          for snake in snakes:
            if snake['head'] != my_head:
              snake_heads.append(snake['head'])
          #this is any part of the snake closest_prey, prey_dist = get_closest_food(my_head, snakes_locations, hazards)
          closest_prey, prey_dist = get_closest_food(my_head, snake_heads, hazards)
          attack_move = move_to_food(my_head, closest_prey, possible_moves)
          #go for prey if they are on the edges
          if closest_prey['x']==0 or closest_prey['x']== board_width or closest_prey['y']== 0 or closest_prey['y']== board_height:
            bravery = 6
            dist_for_interest = 20
          ...
          if my_head['x'] == 0 or my_head['x'] == board_width or my_head['y'] == 0 or my_head['y'] == board_height or attack_dir['x'] == 0 or attack_dir['x'] == board_width or attack_dir['y'] == 0 or attack_dir['y'] == board_height:

            #don't get stuck on the edge of the board when chasing a snake
            if closest_prey['x'] == 1 or closest_prey['x'] == board_width - 1:
              if attack_dir['x'] == 0 or attack_dir['x'] == board_width:
                food_move = food_move
                #if the enemy will trap you on the edge don't go there
            elif closest_prey['y'] == 1 or closest_prey['y'] == board_height - 1:
              if attack_dir['y'] == 0 or attack_dir['y'] == board_height:
                food_move = food_move
            #go for it if brave enough- risk/reward
            elif prey_dist < bravery:
